...
function showComment(key, postId, error, userName, userEmail, needData, bEdit, logId)
{
	...
	?>
	var comment = '', arFiles = [], arDocs = [];

	if(needData == "Y" || bEdit == "Y")
	{
		comment = top["text"+key];
		arFiles = top["arComFiles"+key];
	...
	onLightEditorShow(comment, arFiles, arDocs);
...
function onLightEditorShow(content, arFiles, arDocs)
{
	<?
	...
	var
		...
		editor = window['PlEditor<?=$formParams["FORM_ID"]?>'];
	...
	else if (typeof arDocs == "object" && arDocs.length > 0)
	{
		if (!editor.WDController && !!editor.WDControllerInit)
		{
			BX.addCustomEvent(
				BX.findParent(BX.findChild(BX('<?=$formParams["FORM_ID"]?>'), {'className': 'wduf-selectdialog'}, true, false)),
				'WDLoadFormControllerInit',
				function(obj) {
					onLightEditorShow(content, arFiles, arDocs);
				}
			);
			return editor.WDControllerInit();
		}
		if (editor.WDController && !editor.WDController.onLightEditorShowObj)
		{
			editor.WDController.onLightEditorShowObj = new Array();
			BX.addCustomEvent(
				BX.findParent(BX.findChild(BX('<?=$formParams["FORM_ID"]?>'), {'className': 'wduf-selectdialog'}, true, false)),
				'OnFileUploadSuccess',
				function(result, obj) {
					if (obj.dialogName == 'AttachFileDialog' && BX.util.in_array(result['element_id'], obj['onLightEditorShowObj'])) {
						editor.oEditor.SaveContent();
						var content = editor.oEditor.GetContent();
						content = content.replace(new RegExp('\\&\\#91\\;DOCUMENT ID=(' + result['element_id'] + ')([WIDTHHEIGHT=0-9 ]*)\\&\\#93\\;','gim'), '[DOCUMENT ID=$1$2]');
						editor.oEditor.SetContent(content);
						editor.oEditor.SetEditorContent(editor.oEditor.content);
						editor.oEditor.SetFocus();
						editor.oEditor.AutoResize();
					}
				}
			);
		}
	...
	while ((res = arFiles.pop()) && !!res)
		editor.checkFile(res['id'], res);
	if (!!arDocs && typeof arDocs == "object")
	{
		if (editor.WDController)
			editor.WDController.onLightEditorShowObj = new Array();
		while ((res = arDocs.pop()) && !!res)
		{
			...
			if (editor.WDController)
			{
				...
				arRes.push(tmp);
				editor.WDController.onLightEditorShowObj.push(res);
			}
		}
		if (editor.WDController && arRes.length > 0)
		{
			editor.WDControllerInit('show');
			editor.WDController.agent.values = arRes;
			BX.onCustomEvent(editor.WDController.controller.parentNode, 'OnFileFromDialogSelected', [editor.WDController.agent.values, editor.WDController]);
			editor.WDController.agent.ShowAttachedFiles();
		}
	}

	editor.oEditor.ReInit(content || '');
	editor.oEditor.pFrame.style.height = editor.oEditor.arConfig.height;
	editor.oEditor.ResizeFrame();
	editor.oEditor.AutoResize();
	BX.defer(editor.oEditor.SetFocus, editor.oEditor);
	<?
	}
	?>
}
...
?>
function commentReply(postId, authorId, authorName)
{
	if(authorId > 0)
	{
		if(BX('form_c_del').style.display == 'none' || BX('postId').value.lenght <= 0 || BX('postId').value != postId)
			showComment(0, postId, '', '', '', 'N', 'N');

		var item = {entityId: authorId, name: authorName};
		bPlus = true;
		window['<?=$formParams["LHE"]["jsObjName"]?>'].InsertHTML('+');
		BXfpdSelectCallbackMent(item, 'users', '', '<?=$formParams["FORM_ID"]?>', '<?=$formParams["LHE"]["jsObjName"]?>', true);

		var pos = BX.GetWindowSize();
		var top = pos.innerHeight + pos.scrollTop - 50;

		var fPos = BX.pos(BX('form_c_del'));

		if(fPos.top > top)
			window.scrollTo(pos.left, fPos.top-100);
	}
}
</script>