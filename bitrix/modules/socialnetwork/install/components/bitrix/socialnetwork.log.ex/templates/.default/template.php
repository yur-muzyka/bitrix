			...
			BX.ready(function(){
				...
				if (!$arResult["AJAX_CALL"]):
					...
					BX.addCustomEvent("onPullEvent", BX.delegate(function(module_id,command,params){
						if (module_id == "main" && command == 'user_counter' && params[BX.message('SITE_ID')] && params[BX.message('SITE_ID')][BX.message('sonetLCounterType')])
						{
							__logChangeCounter(params[BX.message('SITE_ID')][BX.message('sonetLCounterType')]);
						}
					}, this));
					<?
				endif;
