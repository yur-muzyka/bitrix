...
function __logChangeCounter(count)
{
	if (parseInt(count) > 0)
	{
		...
		{
			oldVal = BX("sonet_log_counter_2").innerHTML;
		...
		if (BX("sonet_log_counter_2_container"))
		{
			...
			if (oldVal == 0)
			{
				BX("sonet_log_counter_2_container").style.height = "0px";
				BX("sonet_log_counter_2_container").style.overflow = "hidden";
				(new BX.easing({
					duration : 1000,
					start : { height : 0 },
					finish : { height : 40 },
					...
					step : function(state){
						BX("sonet_log_counter_2_container").style.height = state.height + "px";
					},
					complete : function(){
						BX("sonet_log_counter_2_container").style.overflow = "auto";
					}
				})).animate();
			}
		}
