...
$objBitrixOAuth = new CSocServBitrixOAuth($clientId, $clientSecret, $portalURI, $redirectURI, $USER);
$objBitrixOAuth->getEntityOAuth()->addScope(array('task', 'calendar'));
...
if($accessToken != '' && $domain != '' && !$needAuthorize)
{
	...
	$arPlannerTasks = $obApp->getPlannerTasksId();
	...
	if(is_array($arTasksBatch) && isset($arTasksBatch["result"]) && !empty($arTasksBatch["result"]["result"]))
	{
