function JCEC(Params) // Javascript Class Event Calendar
{
	...
	for (ind in this.arSections)
	{
		...
		if (this.bCalDAV && this.arSections[ind].CAL_DAV_CAL && this.arSections[ind].CAL_DAV_CON && this.arConnections && this.arConnections.length > 0)
		{
			for (i in this.arConnections)
			{
				if (this.arConnections[i].id == this.arSections[ind].CAL_DAV_CON)
				{
					this.arSections[ind]['~CAL_DAV_LAST_SYNC'] = this.arConnections[i].last_result;
					break;
				}
			}
		}
...
BuildSectionElement : function(el, bChecked, bSuperpose)
{
	...
	// Determine container
	var
		isGoogle = this.bCalDAV && el.CAL_DAV_CAL && el.CAL_DAV_CON,
