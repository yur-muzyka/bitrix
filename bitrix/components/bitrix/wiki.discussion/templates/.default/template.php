...
if(!empty($arResult['FATAL_MESSAGE'])):
	...
	<div class="wiki_comment_add_link" id="wiki_comment_add_link">
		<a href="#" onclick=" WIKI_show_form_add(true); WIKI_show_link_add(false); return false; ">
			<?=GetMessage('WIKI_COMMENT_ADD')?>
		</a>
	</div>
			...
			if (!!_aTags)
			{
				for (var i = 0; i < _aTags.length; i++)
				{
					BX.bind(
						_aTags[i],
						'click',
						function (e) { WIKI_show_form_add(true); WIKI_show_link_add(false);}
					);
				}
			}
		});
